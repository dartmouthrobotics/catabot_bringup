...
def launch_setup(context, *args, **kwargs):
    ...
    # full_container_name = '/' + container_name_val # this is when using shared
    ...
    record_composable_node = LoadComposableNodes(
        ...
        composable_node_descriptions=[
            ComposableNode(
                package='zed_mcap_recorder',
                plugin='zed_mcap_recorder::ZedMcapRecorder',
                namespace=namespace_val,
                name='recorder_node',
                parameters=[{
                    'bag_path': zed_record_path_val,
                    'recording': True,  # start recording immediately if true
                    'queue_depth_image': 60,
                    'queue_depth_imu': 200,
                    'topics': topics,
                }],
                remappings=[],
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                'use_composable',
                default_value='false',
                description='Enable composable-mode camera-local recording.',
                choices=['true', 'false']),
            DeclareLaunchArgument(
                'zed_record_root_dir',
                default_value='/home/catabot-5/datalog/rosbag2',
                description='Common root directory for composable ZED recorder bags.'),
            DeclareLaunchArgument(
                'zed_record_session',
                default_value=TextSubstitution(text=f"{datetime.now().strftime('%Y-%m-%d-%H-%M-%S')}"),
                description='Shared session folder under zed_record_root_dir for this launch.'),
            DeclareLaunchArgument(
                'sim_mode',
                default_value='false',
